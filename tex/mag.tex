...
\begin{document}
	%\maketitle
	\includepdf[pages={1}]{pd_inz2.pdf}
...
W poniższym listingu kodu oprócz wspomnianych asercji korzystających z operatorów porównania możemy skorzystać z dodatkowych asercji: 
\begin{itemize}
\item BOOST{\_}<LEVEL>, 
\item BOOST{\_}<LEVEL>{\_}BITEWISE{\_}EQUAL, 
\item BOOST{\_}<LEVEL>{\_}CLOSE, 
\item BOOST{\_}<LEVEL>{\_}CLOSE{\_}FRICTION, 
\item BOOST{\_}<LEVEL>{\_}EQUAL{\_}COLLECTIONS, 
\item BOOST{\_}<LEVEL>{\_}EXCEPTION, 
\item BOOST{\_}<LEVEL>{\_}MESSAGE, 
\item BOOST{\_}<LEVEL>{\_}NO{\_}THROW, 
\item BOOST{\_}<LEVEL>{\_}THROW, 
\item BOOST{\_}<LEVEL>{\_}PREDICATE, 
	\end{itemize}
gdzie <LEVEL> może przyjmować jedną z trzech opcji: REQUIRE, CHECK lub WARN.
...
Dodatkowe asercje występujące w tej bibliotece to: 
	\begin{itemize}
\item <LEVEL>{\_}STREQ, 
\item <LEVEL>{\_}STRNE, 
\item <LEVEL>{\_}STRCASEEQ, 
\item <LEVEL>{\_}STRCASENE, 
\item <LEVEL>{\_}THROW, 
\item <LEVEL>{\_}NO{\_}THROW, 
\item <LEVEL>{\_}ANY{\_}THROW, 
\item <LEVEL>{\_}FLOAT{\_}EQ, 
\item <LEVEL>{\_}DOUBLE{\_}EQ, 
\item <LEVEL>{\_}NEAR.
	\end{itemize}
...
<LEVEL>{\_}STRCASEEQ, <LEVEL>{\_}STRCASENE. 
Podobnie jak makra <LEVEL>{\_}STREQ oraz <LEVEL>{\_}STRNE asercje <LEVEL>{\_}STRCASEEQ, <LEVEL>{\_}STRCASENE przyjmują dwa parametry.
...
W dalszej części pracy omówione zostaną makra: 
	\begin{itemize}
\item <LEVEL>{\_}THROW, 
\item <LEVEL>{\_}NO{\_}THROW, 
\item <LEVEL>{\_}ANY{\_}THROW, 
...
Makro BOOST{\_}FIXTURE{\_}TEST{\_}CASE odpowiedziale jest za włączenie uzywania fixtury, przyjmujące ono jako parametry nazwę TC oraz nazwę struktury, której będziemy używać jako fixturę dla definiowanego TC.
...
Aby skorzystać z fixtury dla grupy testów (test suite) należy użyć osobnego makra BOOST{\_}FIXTURE{\_}TEST{\_}SUITE, gdzie jako parametry należy podać nazwę TS oraz nazwę struktury, która będzie pełnić rolę fixtury. 

Makro BOOST{\_}AUTO{\_}TEST{\_}SUITE{\_}END stosujemy tak samo jak do zamknięcia zwykłej TS.
...
Drugi zestaw: TYPED{\_}TEST{\_}CASE{\_}P, REGISTER{\_}TYPED{\_}TEST{\_}CASE{\_}P oraz INSTANTIATE{\_}TYPED{\_}TEST{\_}CASE{\_}P pozwala na zdefiniowanie ciała TC przed zadeklarowaniem typów, na których test będzie przeprowadzany.
...
Pierwsze z nich rozwiązuje problem kiedy operator wypisania jest już wykorzystywany w inny niż spodziewany przez bibliotekę sposób. 

Makro BOOST{\_}TEST{\_}DONT{\_}PRINT{\_}LOG{\_}VALUE pozwala na wyłączenie wypisywania wartości porównywanych obiektów, czyli operator wypisania jest w tym momencie nieużywany i może pełnić inną funkcję.
...
Punkt kontrolny pomaga w łatwy sposób przeanalizować przyczynę błędu poprzez wypisanie wiadomości z ostatniego napotkanego punktu kontrolnego. W przypadku biblioteki Boost unittest jest on realizowany za pomocą makra przyjującego łańcuch znaków do zapamiętania - BOOST{\_}TEST{\_}CHECKPOINT (przykład poniżej).
...
Makro FRIEND{\_}TEST pozwala określić, który z testów ma mieć dostęp do zmiennych niewidocznych publicznie, poprzez określenie go poprzez podanie nazwy TS oraz nazwy TC. Poniżej przykład testowania kodu prywatnego w bibliotece Google test.
...
\lstset{language=C++, caption=Klasa podlegająca testowi - nagłówek klasy, captionpos=b, tabsize=3, frame=lines, keywordstyle=\color{blue}, commentstyle=\color{OliveGreen}, stringstyle=\color{red}, numbers=left, numberstyle=\small,numbersep=5pt, breaklines=true, showstringspaces=false, basicstyle=\footnotesize, emph={label}}
			\lstinputlisting{../gtest_type_assertions/src/simple_calc.h}

\lstset{language=C++, caption=Klasa podlegająca testowi - ciało klasy, captionpos=b, tabsize=3, frame=lines, keywordstyle=\color{blue}, commentstyle=\color{OliveGreen}, stringstyle=\color{red}, numbers=left, numberstyle=\small,numbersep=5pt, breaklines=true, showstringspaces=false, basicstyle=\footnotesize, emph={label}}
			\lstinputlisting{../gtest_type_assertions/src/simple_calc.cpp}
...
\chapter{Podsumowanie}
	...
	\addcontentsline{toc}{chapter}{Bibliografia}
		\bibitem{IEEE754} http://ieeexplore.ieee.org/xpl/
		
		articleDetails.jsp?arnumber\=4610935\&filter=AND(p{\_}Publication{\_}Number:4610933)
		\bibitem{CMake} https://cmake.org/
		\bibitem{DLL} https://en.wikipedia.org/wiki/Dynamic-link{\_}library
	  	\bibitem{Primer} https://github.com/google/googletest/blob/master/googletest/docs/Primer.md
	  	\bibitem{Advanced} https://github.com/google/googletest/blob/master/googletest/docs/AdvancedGuide.md
	  	\bibitem{Boost} http://www.boost.org/doc/libs/1{\_}46{\_}0/libs/test/doc/html/utf.html
	  	\bibitem{devstyle} http://devstyle.pl/2011/08/11/ut-1-co-to-sa-testy-i-po-co-sa-testy-jednostkowe/
